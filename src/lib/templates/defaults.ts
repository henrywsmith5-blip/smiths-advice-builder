...
export function getDefaultTemplate(
  ...
  if (docType === DocType.SOA) {
    if (clientType === ClientType.INDIVIDUAL) {
      return hasCover
        ? { html: SOA_INDIVIDUAL_COVER, css: "" }
        : { html: SOA_INDIVIDUAL_NOCOVER, css: "" };
    }
    if (clientType === ClientType.PARTNER) {
      return hasCover
        ? { html: SOA_PARTNER_COVER, css: "" }
        : { html: SOA_PARTNER_NOCOVER, css: "" };
    }
  }
...
// ─── Shared CSS for all SOA templates ───
const SOA_CSS = `
  ...
  *{margin:0;padding:0;box-sizing:border-box;} @page{size:A4;margin:18mm 16mm 22mm 16mm;}
  ...
  .signature-grid{display:grid;grid-template-columns:1fr 1fr;gap:24px;margin:24px 0;page-break-inside:avoid;}
  .sig-box{border:1px solid var(--border);border-radius:6px;padding:22px 24px;margin-bottom:16px;} .sig-box .sig-name{font-weight:700;color:var(--dark);font-size:10.5pt;margin-bottom:24px;} .sig-line{border-bottom:1px solid var(--dark);height:1px;margin-bottom:6px;margin-top:32px;} .sig-label{font-size:8.5pt;color:var(--muted);text-transform:uppercase;letter-spacing:0.3px;} .adviser-sig{border:1px solid var(--border);border-radius:6px;padding:22px 24px;margin:16px 0;page-break-inside:avoid;}
...
const SOA_HEAD = (title: string) => `<!DOCTYPE html>
...
<style>${SOA_CSS}</style></head><body>
<div class="accent-bar"></div><div class="page">
  <div class="header"><img class="logo" src="https://images.squarespace-cdn.com/content/v1/6033fe3152058c67d1e84e7f/1614286673894-ZH98E19GRUKA55E6Z17W/Smiths_wide_withouttagline_RGB_COLOUR-300dpi.jpg?format=1500w" alt="Smiths Insurance &amp; KiwiSaver"><span class="doc-type">Statement of Advice</span></div>
...
// ════════════════════════════════════════════════════════════
// 1. SOA INDIVIDUAL — NO COVER (centered new cover, no migration)
// ════════════════════════════════════════════════════════════
const SOA_INDIVIDUAL_NOCOVER = SOA_HEAD("Statement of Advice — Individual — New Cover") + `
  <div class="schedule-card"><h3>Engagement Schedule</h3><table class="schedule-table"><tr><td>Financial Advice Provider</td><td>Craig Smith Business Services Limited</td></tr><tr><td>FAP Licence Number</td><td>FSP712931 (Class 2 — FMA)</td></tr><tr><td>Trading Name</td><td>Smiths Insurance &amp; KiwiSaver</td></tr><tr><td>Individual Adviser</td><td>{{ ADVISER_NAME }} ({{ ADVISER_FSP }})</td></tr><tr><td>Engagement Date</td><td><span class="placeholder">{{ ENGAGEMENT_DATE }}</span></td></tr></table></div>
  <div class="prepared-for"><div class="info-pair"><span class="label">Prepared For:</span><span class="value">{{ CLIENT_NAME }}</span></div><div class="info-pair"><span class="label">Date:</span><span class="value">{{ DATE }}</span></div></div>

  <div class="section-heading"><div class="num">1</div><h2>Parties</h2></div>
  <div class="info-card"><h4>Client Details</h4><table class="schedule-table"><tr><td>Client Name</td><td>{{ CLIENT_NAME }}</td></tr><tr><td>Email</td><td>{{ CLIENT_EMAIL }}</td></tr><tr><td>Phone</td><td>{{ CLIENT_PHONE }}</td></tr></table></div>

  <div class="section-heading"><div class="num">2</div><h2>Scope of Advice</h2></div>
  <div class="sub-heading">{{ CLIENT_NAME }}</div>
  <ul class="checkbox-list"><li>Life Insurance</li><li>Trauma Insurance</li><li>Total &amp; Permanent Disability Insurance</li><li>Income Protection Insurance</li><li>Health Insurance</li></ul>
  ...
  <div class="callout-box"><strong>Preferred Insurer:</strong> AIA Life — excellent policy benefits, reasonable premiums, modular benefits.</div>

  <div class="section-heading"><div class="num">4</div><h2>Out of Scope</h2></div>
  <ul class="styled-list"><li>Fire and general insurance — referred to Aon Insurance Brokers</li><li>Mortgages, lending, or budgeting services</li><li>Tax, legal, or accounting services</li><li>Estate planning or business succession</li></ul>

  <div class="section-heading"><div class="num">5</div><h2>Client Responsibilities</h2></div>
  <ul class="styled-list"><li>Provide complete and accurate information</li><li>Disclose all information that could influence an insurer's decision</li><li>Inform us if your circumstances change</li></ul>

  <div class="section-heading"><div class="num">6</div><h2>Fees &amp; Commission</h2></div>
  <p class="body-text">No fee charged. Smiths receives commission from insurers when policies are placed.</p>

  <div class="section-heading"><div class="num">7</div><h2>Conflicts of Interest</h2></div>
  <p class="body-text">We manage conflicts by following a documented advice process and comparing products across multiple providers.</p>

  <!-- 8. RECOMMENDED COVERS — NEW COVER ONLY (CENTRED) -->
  ...
  <div class="sub-heading" style="text-decoration:underline;">Recommended Covers — {{ CLIENT_NAME }}</div>
  <p class="body-text"><em>Recommended new cover with {{ NEW_INSURER }}</em></p>
  <div class="new-cover-centered">
    <div class="side-header">Recommended Cover — {{ NEW_INSURER }}</div>
  ...
  <!-- 9. REASONS -->
  <div class="section-heading"><div class="num">9</div><h2>Reasons for Recommendations</h2></div>
  {% if LIFE_INCLUDED %}<div class="sub-heading">Life Cover</div><div class="info-card">{{ REASON_LIFE_COVER }}</div>{% endif %}
  {% if TRAUMA_INCLUDED %}<div class="sub-heading">Trauma Cover</div><div class="info-card">{{ REASON_TRAUMA }}</div>{% endif %}
  {% if TPD_INCLUDED %}<div class="sub-heading">TPD Cover</div><div class="info-card">{{ REASON_TPD }}</div>{% endif %}
  {% if INCOME_MP_INCLUDED %}<div class="sub-heading">Income &amp; Mortgage Protection</div><div class="info-card">{{ REASON_INCOME_MORTGAGE }}</div>{% endif %}

  <div class="section-heading"><div class="num">10</div><h2>Ongoing Service</h2></div>
  <p class="body-text">Following policy placement, we provide ongoing service at no additional fee.</p>

  <div class="section-heading"><div class="num">11</div><h2>Privacy and Records</h2></div>
  <p class="body-text">Records retained for a minimum of seven years per regulatory requirements.</p>

  <div class="section-heading"><div class="num">12</div><h2>Complaints and Disputes</h2></div>
  <p class="body-text">Henry Smith: henry@smiths.net.nz | 027 344 5255<br>Craig Smith: craig@smiths.net.nz | 0274 293 939</p>
  <p class="body-text"><strong>FDRS:</strong> fdrs.org.nz | enquiries@fdrs.org.nz</p>

  <div class="section-heading page-break"><div class="num">13</div><h2>Client Acknowledgement &amp; Sign-Off</h2></div>
  <div class="info-card" style="background:var(--card-bg);"><p class="body-text">I, <strong>{{ CLIENT_NAME }}</strong>, have read and understand this statement of advice.</p><ul class="checkbox-list" style="margin:16px 0;"><li><strong>ACCEPT</strong></li><li><strong>DECLINE</strong></li><li><strong>MODIFY</strong> (see notes below)</li></ul></div>
  <div class="sig-box"><div class="sig-name">{{ CLIENT_NAME }}</div><div class="sig-line"></div><div class="sig-label">Client Signature</div><div class="sig-line"></div><div class="sig-label">Date</div></div>
  <div class="adviser-sig"><p style="margin-bottom:18px;"><strong>Craig Smith</strong> | FAP License #33042</p><div class="sig-line"></div><div class="sig-label">Signature &amp; Date</div></div>
` + SOA_FOOTER;

// ════════════════════════════════════════════════════════════
// 2. SOA INDIVIDUAL — WITH COVER (comparison layout + migration + declaration)
// ════════════════════════════════════════════════════════════
const SOA_INDIVIDUAL_COVER = SOA_HEAD("Statement of Advice — Individual — Existing Cover") + `
  <div class="schedule-card"><h3>Engagement Schedule</h3><table class="schedule-table"><tr><td>Financial Advice Provider</td><td>Craig Smith Business Services Limited</td></tr><tr><td>FAP Licence Number</td><td>FSP712931 (Class 2 — FMA)</td></tr><tr><td>Trading Name</td><td>Smiths Insurance &amp; KiwiSaver</td></tr><tr><td>Individual Adviser</td><td>{{ ADVISER_NAME }} ({{ ADVISER_FSP }})</td></tr><tr><td>Engagement Date</td><td><span class="placeholder">{{ ENGAGEMENT_DATE }}</span></td></tr></table></div>
  <div class="prepared-for"><div class="info-pair"><span class="label">Prepared For:</span><span class="value">{{ CLIENT_NAME }}</span></div><div class="info-pair"><span class="label">Date:</span><span class="value">{{ DATE }}</span></div></div>
  ...
  <!-- 8. RECOMMENDED COVERS — COMPARISON LAYOUT -->
  <div class="section-heading page-break"><div class="num">8</div><h2>Recommended Covers</h2></div>
  <p class="body-text"><strong>Date:</strong> {{ SIGNOFF_DATE }}</p>
  <div class="sub-heading" style="text-decoration:underline;">Agreed Covers — {{ CLIENT_NAME }}</div>
  <p class="body-text"><em>Summary of changes from {{ OLD_INSURER }} to {{ NEW_INSURER }}</em></p>
  <div class="comparison-wrapper">
    <div class="comparison-side old-side"><div class="side-header">Current Cover — {{ OLD_INSURER }}</div><table><tr><th>Cover Type</th><th>Sum Insured</th></tr><tr><td>Life</td><td class="empty">{{ OLD_LIFE }}</td></tr><tr><td>Trauma</td><td class="empty">{{ OLD_TRAUMA }}</td></tr><tr><td>T.P.D.</td><td class="empty">{{ OLD_TPD }}</td></tr><tr><td>Income Protection</td><td class="empty">{{ OLD_IP }}</td></tr><tr><td>Mortgage Protection</td><td class="empty">{{ OLD_MP }}</td></tr><tr><td>Accidental Injury</td><td class="empty">{{ OLD_AIC }}</td></tr><tr><td>Premium Cover</td><td class="empty">{{ OLD_PREMIUM_COVER }}</td></tr></table></div>
    <div class="comparison-side new-side"><div class="side-header">Recommended Cover — {{ NEW_INSURER }}</div><table><tr><th>Cover Type</th><th>Sum Insured</th></tr><tr><td>Life</td><td class="empty">{{ NEW_LIFE }}</td></tr><tr><td>Progressive Care / Trauma</td><td class="empty">{{ NEW_TRAUMA }}</td></tr><tr><td>T.P.D.</td><td class="empty">{{ NEW_TPD }}</td></tr><tr><td>Income Protection</td><td class="empty">{{ NEW_IP }}</td></tr><tr><td>Mortgage Protection</td><td class="empty">{{ NEW_MP }}</td></tr><tr><td>Accidental Injury</td><td class="empty">{{ NEW_AIC }}</td></tr><tr><td>Premium Cover</td><td class="empty">{{ NEW_PREMIUM_COVER }}</td></tr></table></div>
  </div>
  <div class="sub-heading">Premium Summary</div>
  ...
  {% if TRAUMA_INCLUDED %}<div class="sub-heading">Trauma</div><div class="pros-cons"><div class="col pro"><div class="col-header">Pros</div><div class="col-body">{{ TRAUMA_PROS }}</div></div><div class="col con"><div class="col-header">Cons</div><div class="col-body">{{ TRAUMA_CONS }}</div></div></div>{% endif %}
  {% if TPD_INCLUDED %}<div class="sub-heading">TPD</div><div class="pros-cons"><div class="col pro"><div class="col-header">Pros</div><div class="col-body">{{ TPD_PROS }}</div></div><div class="col con"><div class="col-header">Cons</div><div class="col-body">{{ TPD_CONS }}</div></div></div>{% endif %}
  ...
  <!-- 10. REASONS -->
  <div class="section-heading page-break"><div class="num">10</div><h2>Reasons for Recommendations</h2></div>
  {% if LIFE_INCLUDED %}<div class="sub-heading">Life Cover</div><div class="info-card">{{ REASON_LIFE_COVER }}</div>{% endif %}
  {% if TRAUMA_INCLUDED %}<div class="sub-heading">Trauma Cover</div><div class="info-card">{{ REASON_TRAUMA }}</div>{% endif %}
  {% if TPD_INCLUDED %}<div class="sub-heading">TPD Cover</div><div class="info-card">{{ REASON_TPD }}</div>{% endif %}
  {% if INCOME_MP_INCLUDED %}<div class="sub-heading">Income &amp; Mortgage Protection</div><div class="info-card">{{ REASON_INCOME_MORTGAGE }}</div>{% endif %}

  <div class="section-heading"><div class="num">11</div><h2>Ongoing Service</h2></div>
  <p class="body-text">Following policy placement, we provide ongoing service at no additional fee.</p>

  <div class="section-heading"><div class="num">12</div><h2>Privacy and Records</h2></div>
  <p class="body-text">Records retained for a minimum of seven years per regulatory requirements.</p>

  <!-- 13. DECLARATION OF CONTINUED GOOD HEALTH -->
  <div class="section-heading page-break"><div class="num">13</div><h2>Declaration of Continued Good Health</h2></div>
  <p class="body-text">As your Financial Adviser, I want to ensure the cover applied for can operate correctly at claim time.</p>
  <ul class="checkbox-list"><li>No therapy or treatment since the application</li><li>No treatment not already disclosed</li><li>No changes in health since the application</li><li>I provided all answers myself</li><li>The adviser explained additional exclusions</li><li>The adviser explained payment dates and premiums</li><li>The adviser explained non-disclosure risks</li><li>The adviser explained differences between old and new policy</li></ul>
  <div class="warning-box"><div class="warning-label">Important</div><p>The adviser explained the risks of shifting insurance cover. {{ NEW_INSURER }} have underwritten the insurance risk as of today.</p></div>

  <div class="section-heading"><div class="num">14</div><h2>Complaints and Disputes</h2></div>
  <p class="body-text">Henry Smith: henry@smiths.net.nz | 027 344 5255<br>Craig Smith: craig@smiths.net.nz | 0274 293 939</p>
  <p class="body-text"><strong>FDRS:</strong> fdrs.org.nz | enquiries@fdrs.org.nz</p>

  <div class="section-heading page-break"><div class="num">15</div><h2>Client Acknowledgement &amp; Sign-Off</h2></div>
  <div class="info-card" style="background:var(--card-bg);"><p class="body-text">I, <strong>{{ CLIENT_NAME }}</strong>, have read and understand this statement of advice.</p><ul class="checkbox-list" style="margin:16px 0;"><li><strong>ACCEPT</strong></li><li><strong>DECLINE</strong></li><li><strong>MODIFY</strong> (see notes below)</li></ul></div>
  <div class="sig-box"><div class="sig-name">{{ CLIENT_NAME }}</div><div class="sig-line"></div><div class="sig-label">Client Signature</div><div class="sig-line"></div><div class="sig-label">Date</div></div>
  <div class="adviser-sig"><p style="margin-bottom:18px;"><strong>Craig Smith</strong> | FAP License #33042</p><div class="sig-line"></div><div class="sig-label">Signature &amp; Date</div></div>
` + SOA_FOOTER;
...
// ════════════════════════════════════════════════════════════
// 3. SOA PARTNER — NO COVER (centred per-client tables)
// ════════════════════════════════════════════════════════════
const SOA_PARTNER_NOCOVER = SOA_HEAD("Statement of Advice — Partner — New Cover") + `
  <div class="schedule-card"><h3>Engagement Schedule</h3><table class="schedule-table"><tr><td>Financial Advice Provider</td><td>Craig Smith Business Services Limited</td></tr><tr><td>Individual Adviser</td><td>{{ ADVISER_NAME }} ({{ ADVISER_FSP }})</td></tr><tr><td>Engagement Date</td><td><span class="placeholder">{{ ENGAGEMENT_DATE }}</span></td></tr></table></div>
  <div class="prepared-for"><div class="info-pair"><span class="label">Prepared For:</span><span class="value">{{ CLIENT_A_NAME }} &amp; {{ CLIENT_B_NAME }}</span></div><div class="info-pair"><span class="label">Date:</span><span class="value">{{ DATE }}</span></div></div>

  <div class="section-heading"><div class="num">1</div><h2>Parties</h2></div>
  <div class="info-card"><h4>Client Details</h4><table class="schedule-table"><tr><td>Client Names</td><td>{{ CLIENT_A_NAME }} &amp; {{ CLIENT_B_NAME }}</td></tr><tr><td>Email</td><td>{{ CLIENT_EMAIL }}</td></tr><tr><td>Phone</td><td>{{ CLIENT_PHONE }}</td></tr></table></div>

  <div class="section-heading"><div class="num">2</div><h2>Scope of Advice</h2></div>
  <div class="sub-heading">{{ CLIENT_A_NAME }}</div>
  <ul class="checkbox-list"><li>Life Insurance</li><li>Trauma Insurance</li><li>TPD Insurance</li><li>Income Protection</li><li>Health Insurance</li></ul>
  <div class="sub-heading">{{ CLIENT_B_NAME }}</div>
  <ul class="checkbox-list"><li>Life Insurance</li><li>Trauma Insurance</li><li>TPD Insurance</li><li>Income Protection</li><li>Health Insurance</li></ul>

  <!-- 8. RECOMMENDED COVERS — CENTRED PER CLIENT -->
  <div class="section-heading page-break"><div class="num">8</div><h2>Recommended Covers</h2></div>
  <p class="body-text"><strong>Date:</strong> {{ SIGNOFF_DATE }}</p>
...
// ════════════════════════════════════════════════════════════
// 4. SOA PARTNER — WITH COVER (comparison per client + migration)
// ════════════════════════════════════════════════════════════
const SOA_PARTNER_COVER = SOA_HEAD("Statement of Advice — Partner — Existing Cover") + `
  <div class="schedule-card"><h3>Engagement Schedule</h3><table class="schedule-table"><tr><td>Financial Advice Provider</td><td>Craig Smith Business Services Limited</td></tr><tr><td>Individual Adviser</td><td>{{ ADVISER_NAME }} ({{ ADVISER_FSP }})</td></tr><tr><td>Engagement Date</td><td><span class="placeholder">{{ ENGAGEMENT_DATE }}</span></td></tr></table></div>
  <div class="prepared-for"><div class="info-pair"><span class="label">Prepared For:</span><span class="value">{{ CLIENT_A_NAME }} &amp; {{ CLIENT_B_NAME }}</span></div><div class="info-pair"><span class="label">Date:</span><span class="value">{{ DATE }}</span></div></div>
  ...
  <!-- 8. RECOMMENDED COVERS — COMPARISON PER CLIENT -->
  <div class="section-heading page-break"><div class="num">8</div><h2>Recommended Covers</h2></div>
  <p class="body-text"><strong>Date:</strong> {{ SIGNOFF_DATE }}</p>
  ...
  <!-- DECLARATION -->
  <div class="section-heading page-break"><div class="num">13</div><h2>Declaration of Continued Good Health</h2></div>
  <ul class="checkbox-list"><li>No therapy or treatment since the application</li><li>No changes in health</li><li>The adviser explained additional exclusions</li><li>The adviser explained differences between old and new policy</li></ul>
  <div class="warning-box"><div class="warning-label">Important</div><p>The adviser explained the risks of shifting insurance cover.</p></div>
...
<html lang="en"><head><meta charset="UTF-8"><title>Record of Advice — Smiths</title>
...
<style>:root{--accent:#C08B6F;--dark:#2C2C2C;--body:#444;--muted:#888;--card-bg:#F7F5F2;--border:#E8E4DF;--font-display:'Playfair Display',Georgia,serif;--font-body:'Source Sans 3',sans-serif;}*{margin:0;padding:0;box-sizing:border-box;}@page{size:A4;margin:18mm 16mm 22mm 16mm;}body{font-family:var(--font-body);font-size:10.5pt;line-height:1.6;color:var(--body);background:#fff;}.page{max-width:800px;margin:0 auto;padding:0 40px;}.accent-bar{width:100%;height:8px;background:linear-gradient(90deg,var(--accent),#D4A88E);}.header{display:flex;justify-content:space-between;align-items:center;padding:32px 0 24px;border-bottom:1px solid var(--border);}.header img{height:48px;}.title-block{text-align:center;padding:48px 0 36px;}.title-block h1{font-family:var(--font-display);font-size:28pt;color:var(--dark);}.section-heading{display:flex;align-items:center;gap:14px;margin:40px 0 18px;}.section-heading .num{width:30px;height:30px;background:var(--accent);color:#fff;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:12pt;}.section-heading h2{font-family:var(--font-display);font-size:15pt;color:var(--dark);text-transform:uppercase;}.body-text{margin-bottom:14px;}.info-card{background:var(--card-bg);border:1px solid var(--border);border-radius:6px;padding:22px 28px;margin-bottom:20px;}.footer{border-top:2px solid var(--accent);padding-top:12px;margin-top:40px;display:flex;justify-content:space-between;font-size:8pt;color:var(--muted);}</style>
  ...
  {% if ROA_DEVIATIONS %}<div class="section-heading"><div class="num">2</div><h2>Deviations from SOA</h2></div><div class="info-card">{{ ROA_DEVIATIONS }}</div>{% endif %}
